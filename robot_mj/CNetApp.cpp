...
bool CUser::ProHostIngameMsgByStream(Packet * packet) {
	...
	case INGAME_PRE_TANK_READY: //非房主发
	{
		if (m_pHost && m_pHost->nHostId == m_nUserId) {
			SetTimer(ON_INGAME_PRE_TANK_START, 1000 + random() % 1000);
		}
	}
		return true;
		// 通知所有clientslavers 开始游戏
	case INGAME_PRE_TANK_START: //房主发开始
	{
		SetTimer(ON_INGAME_TANK_START, 1000 + random() % 1000);
	}
		return true;
		// 开始游戏计时
	case INGAME_TANK_START: //互发
	{
		SetTimer(ON_INGAME_TANK_FIRE, 1000 + random() % 1000);

	}
		return true;
		// 更新自己的tank参数
	case INGAME_STATE_TANK_POSITION: //位置参数，炮管角度。
	{

	}
		return true;
		// 更新自己的ap
	case INGAME_STATE_TANK_AP: //更新血量
	{

	}
		return true;
		// 通知开火事件
	case INGAME_EVENT_TANK_FIRE: //我开火，目标，
	{
		SetTimer(ON_INGAME_TANK_MOVE, 1000 + random() % 1000);

	}
		return true;
		// 通知重生事件
	case INGAME_EVENT_TANK_REBORN: //主机通知，有人复活
	{

	}
		return true;
		// 通知吃道具事件
	case INGAME_EVENT_TANK_PROPS: //不一定自己吃到
	{

	}
		return true;
		// 通知炸毁事件
	case INGAME_EVENT_TANK_DESTORY: //主机通知。
	{

	}
		return true;
		// INGAME内购买事件
	case INGAME_EVENT_BUY: {

	}
		return true;
		// gameover
	case INGAME_TANK_GAMEOVER: //主机通知
	{
		SetTimer(ON_LEAVE_HOST_TIME, 1000 + random() % 1000);
	}
		return true;
		// 通知退出游戏，但不退出host
	case INGAME_EXIT_INGAME_NOTIFY: //通知其它人自己退出
	{

	}
		return true;
		// 聊天消息
	case INGAME_MESSAGES: {

	}
		return true;
		// 赛后统计互传事件
	case INGAME_END2OTHERS_STATEINFO: {

	}
		return true;
		// 房主改变通知
	case INGAME_HOSTMASTER_CHANGE: {

	}
		return true;
		// 中途有palyer加入
	case INGAME_MIDWAY_NEW_COMING: {

	}
		return true;
		// 通知中途加入的玩家ingame已有统计信息
	case INGAME_MIDWAY_DATA_NOTIFY: {
