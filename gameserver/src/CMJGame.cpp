...
bool CMJGame::ProHostMsgByStream(unsigned int uid, unsigned char * data, unsigned int len)
{
	return true;
...
//		SendData(&bsData, m_ServerAddr);
	...
	case ONTIMER_WAIT_ZHUOZHUANG: {
		printf("ontime=====zhuozhuang======>\n");

		unsigned int uid = m_nIndexlist[m_nCurIndex];
		RakNet::BitStream bsData;
		bsData.Write((unsigned char) PT_HOST_MESSAGE);
		bsData.Write((unsigned int) PT_MJ_ZHUOZHUANG_REQ);
		bsData.Write((unsigned int) 1);
		SendBitStream2Myself(PT_HOST_MESSAGE, uid, &bsData);
	}
		break;

	case ONTIMER_WAIT_LAZHUANG: {
		printf("ontime=====lazhuang======>\n");
		unsigned int uid = m_nIndexlist[m_nCurIndex];
		RakNet::BitStream bsData;
		bsData.Write((unsigned char) PT_HOST_MESSAGE);
		bsData.Write((unsigned int) PT_MJ_LAZHUANG_REQ);
		bsData.Write((unsigned int) 0);
		SendBitStream2Myself(PT_HOST_MESSAGE, uid, &bsData);
	...
	case ONTIMER_WAIT_DAPAI: {
		...
		bsData.Write((unsigned int) ((CMJUser *)m_Userlist[uid])->m_Cardlist.begin()->first);
		SendBitStream2Myself(PT_HOST_MESSAGE, uid, &bsData);
	}
		break;
	case ONTIMER_WAIT_RECONNECT: {
